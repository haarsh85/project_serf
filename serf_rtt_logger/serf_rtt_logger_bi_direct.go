...
func main() {
	...
	for currentIdx, currentNet := range networks {
		...
		// Inter-network pings with all other networks
		for otherIdx := 0; otherIdx < len(networks); otherIdx++ {
			if otherIdx == currentIdx {
				continue
			}
			targetNet := networks[otherIdx]
			header := fmt.Sprintf("=== Inter-network pings %s → %s ===\n",
				currentNet.Name, targetNet.Name)
			serfLog.WriteString(header)
			fmt.Print(header)
			processInter(currentNet, targetNet, serfLog)
			// serfLog.WriteString("\n")
			// fmt.Print("\n")
		}
	}
