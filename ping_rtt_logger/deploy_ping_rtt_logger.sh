...
go_binary="ping_rtt_logger"  # Compiled Go binary
...
deploy_coordinate_binary() {
  for container in "${containers[@]}"; do
    ...
    echo "Copying coordinate binary to $container..."
    ...
    #Copy the networks_nodes
    docker cp networks_nodes.txt "$container":"$destination_dir"/ || { echo "Failed to copy Go binary to $container"; exit 1; }
    echo "coordinate binary successfully set up on $container."
  done
}
