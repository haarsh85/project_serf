...
if len(eucl_dists) >= 2:
    ...
    debug_print("[Warning] Not enough points for regression; falling back to linear mapping.")
...
# === Recursive Hilbert search with outside check optimization ===
intervals = []
...
def hilbert_recurse(region_min, region_max, level):
    ...
    debug_print(f"Recursion level {level}: region_min={region_min}, region_max={region_max}")

    if is_fully_inside(region_min, region_max, bbox):
        interval = region_hilbert_interval(region_min, region_max)
        intervals.append(interval)
        debug_print(f"Fully inside region at level {level}, added interval: {interval}")
        return

    if level == BITS:
        midpoint = [(a + b) // 2 for a, b in zip(region_min, region_max)]
        h = hilbert.distance_from_point(midpoint)
        intervals.append((h, h))
        debug_print(f"Leaf node interval added: ({h}, {h})")
        return
...
msg_candidates_count = f"Candidate nodes found: {len(candidates)}"
msg_candidates_list = f"Candidates: {sorted(candidates)}"
print(msg_candidates_count)
print(msg_candidates_list)
debug_print(msg_candidates_count)
debug_print(msg_candidates_list)
