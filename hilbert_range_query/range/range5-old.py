...
#     height_a, height_b = a["Height"], b["Height"]
#     adj_a, adj_b = a["Adjustment"], b["Adjustment"]

#     euclidean = np.linalg.norm(np.array(vec_a) - np.array(vec_b))
#     rtt = euclidean + height_a + height_b
#     adjusted = rtt + adj_a + adj_b
#     return max(rtt, adjusted) * 1000  # seconds to ms

# def compute_ground_truth(query_name, T):
#     q = nodes[query_name]
#     return {
#         name for name in all_node_names if name != query_name
#         and compute_rtt(q, nodes[name]) <= T
#     }
